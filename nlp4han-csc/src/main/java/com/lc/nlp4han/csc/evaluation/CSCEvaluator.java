...
public class CSCEvaluator extends AbstractEvaluation
{
	...
	private void validate()
	{
		...
		if ((originalSize * goldSize * resultSize) != 0)
		{
			...
			{
				System.err.println("数据量不等！\noriginalSize = " + originalSize + "goldSize = " + goldSize + "resultSize = "
						+ resultSize);
				System.exit(0);
			}
		}
	...
	protected HashSet<Integer> getDTP()
	{
		...
		{ // 遍历黄金标准中的错误句
			...
			for (int i = 0; i < errorSentence.size(); i++)
			{ // 遍历错误句的每一个字
			...
			int count = 0;
			for (int j : errorLocations)
			{ // 记录该句子所有错误字的位置，被系统检测出个数
	...
	protected HashSet<Integer> getDFP()
	{
		...
		{ // 遍历系统检测结果中的错误句
			...
			int count = 0;
			for (int j : reslutLocations)
			{ // 记录系统给出的所有错误字的位置，黄金标准是否也认为有错的个数
