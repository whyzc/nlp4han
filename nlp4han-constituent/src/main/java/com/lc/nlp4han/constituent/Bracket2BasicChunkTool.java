...
public class Bracket2BasicChunkTool
{
	...
	{
		String[] CHUNKS = {"ADJP", "ADVP", "CP", "DNP", "DP", "DVP", "FRAG", "LCP", "LST", "NP", "PP", "PRN", "QP", "VP", "CLP", "UCP"};
		StringBuilder result = new StringBuilder();
		List<TreeNode> leaves = TreeNodeUtil.getAllLeafNodes(root);
		for (int i=0 ; i<leaves.size() ; i++)
		{
			TreeNode leaf = leaves.get(i);
			if (leaf.getParent().getNodeName().equals("PU") && TreeNodeUtil.getFirstNodeUpWithSpecifiedName(leaf, "FRAG")==null)
			{
				...
				if (c != null)
				{
					...
					else
					{
						...
						for (int j=0 ; j<n ; j++)
						{
							if (j!=0)
								result.append(" ");
							result.append(ls.get(start+j).getNodeName()+"/"+ls.get(start+j).getParent().getNodeName());
						}
	...
	private static int chunkNum(TreeNode node, int index, String[] chunks)
	{
		...
		while(num > 1)
		{//最后一个为标点，删除
			if (leaves.get(index+num-1).getParent().getNodeName().equals("PU")&&TreeNodeUtil.getFirstNodeUpWithSpecifiedName(leaves.get(index+num-1), "FRAG")==null)
				num--;
			else
				break;
		}
	...
	public static void main(String[] args) throws IOException
	{
		...
		else
		{
			...
			System.out.println(processSample(arg[0]));
	...
	{
		String usage = "\t1) -path DOC_PATH [-encoding ENCODING] [-save DOC_PATH]\n"
				+ "\t2) -sample bracket_expression\n\n";
		...
		if (args.length>0 && "-sample".equals(args[0]))
		{
			...
			sample = sb.substring(0, sb.length()-1);
		}

		if (docPath == null && sample == null)
		{
