...
public class BracketTest {
	...
	public void BracketConvertTest() throws IOException {
		...
		/*测试修改一，在括号表达式转化为树节点的形式时，将-LRB-，-RRB-转换为"(",")"
		 * 测试在树形式时左右括号是否为"("")"的形式
		 */
		...
		/*
		 * 测试七,printTree，输出树时，它的空格数目不好计算，而且只要看它的括号表达式中的左右括号是否转换，所以，我直接打印出来观看,其结果如下
		 * (IP
	           (NP (NR 我) (PU -LRB-) (NN 李斯) (PU -RRB-))
	           (VP (Verb 是)
		              (NP (PU -LRB-) (NN 大秦) (PU -RRB-) (NR 丞相)))
	           (PU 。))
		 */
