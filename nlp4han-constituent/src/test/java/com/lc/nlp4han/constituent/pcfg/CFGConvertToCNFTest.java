...
public class CFGConvertToCNFTest {
   ...
   public void BeforeConvert() {
	   ...
	   convert=new ConvertCFGToCNF(cfg);
	   
   }
   /*
    * 添加新的起始符DuS,新的规则DuS->S
    */
   @Test
   public void addNewStartSymbolTest() {
	   convert.addNewStartSymbol(cfg);
	   cnf=convert.getCNF();
	   //起始符替换
	   ...
	   //新的规则
	   Assert.assertTrue(cnf.getRuleSet().contains(new RewriteRule("Du"+"S","S")));
   }
   /*
    * 如果规则右侧的字符串个数大于或等于三个则递归减少至两个，在递归前将右侧的终结符转换为非终结符
    * 以new RewriteRule("S","A","B","c","D","e","g")的转换来验证
    */   
   @Test
   public void ReduceNumOfRHSTest() {
	   convert.ReduceNumOfRHS(cfg);;
	   cnf=convert.getCNF();
   ...
   public void ConvertMixRuleToCNFTest() {
	   convert.ConvertMixRuleToCNF(cfg);
	   cnf=convert.getCNF();
	   HashSet<RewriteRule> set=new HashSet<RewriteRule>();
   ...
   public void RemoveUnitProductionTest() {
	   ...
	   addSet.add(new RewriteRule("B","b"));
	   addSet.add(new RewriteRule("E","e"));
