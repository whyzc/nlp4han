	...
	public void update(String[] tokens, String[] refChunkTags, String[] preChunkTags)
	{
		...
		{// 遍历样本中的每个词,统计样本中每类组块标记标准数量与预测数量
			...
			if (refChunkTag.equals("O") || refChunkTag.split("_")[1].equals("B")
					|| refChunkTag.split("_")[1].equals("S"))
			{// 统计参考样本中各个组块数量，及预测正确的数量
				...
				if (refChunkTag.equals("O") || refChunkTag.split("_")[1].equals("S"))
				{// 当前词的组块标记为O或*_S
					String refChunk = "";
					...
					if (referenceChunk2Count.containsKey(refChunk))
						referenceChunk2Count.put(refChunk, referenceChunk2Count.get(refChunk) + 1);
					else
						referenceChunk2Count.put(refChunk, 1L);
					...
					{// 非组块被预测正确，进行统计
						if (correctTag2Count.containsKey(preChunk))
							correctTag2Count.put(preChunk, correctTag2Count.get(preChunk) + 1);
						else
							correctTag2Count.put(preChunk, 1L);
			...
			if (preChunkTag.equals("O") || preChunkTag.split("_")[1].equals("B")
					|| preChunkTag.split("_")[1].equals("S"))
			{
				...
				if (preChunkTag.equals("O") || preChunkTag.split("_")[1].equals("S"))
				{// 当前词的组块预测标记为O或*_S
					String preChunk = "";
					if (preChunkTag.equals("O"))
						preChunk = preChunkTag;
					...
					if (predictChunk2Count.containsKey(preChunk))
						predictChunk2Count.put(preChunk, predictChunk2Count.get(preChunk) + 1);
					else
						predictChunk2Count.put(preChunk, 1L);
				}
				else// 当前词的组块预测标记为*_B
